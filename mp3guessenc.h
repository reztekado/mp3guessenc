...
/* extra bytes physically reserved after mp3g_storage[LARGEST_BUFFER-1].
 * When the head-of-file metadata scan fills the buffer completely, it also
 * peeks this many further real bytes from the file into the margin, so a
 * tag signature (ID3v2/APEv2/RIFF) found close to the tail of the just-read
 * window can still have its whole fixed-size record read safely instead of
 * running past the buffer. See ID3V2_HEAD_SEARCH_SLACK / APETAGX_HEAD_SEARCH_SLACK
 * / WAVERIFF_SEARCH_SLACK in tags.h, whose largest value this must cover. */
#define TAG_SEARCH_MARGIN                       32
